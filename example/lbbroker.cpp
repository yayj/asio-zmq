...
public:
    req_client(boost::asio::io_service& ios, boost::asio::zmq::context& ctx, int id)
        ...
        requester_.set_option(
            boost::asio::zmq::socket_option::identity(
...
private:
    ...
    void handle_worker_ready(boost::system::error_code const& ec,
                             message_ptr buff) {
    ...
    void handle_client_requested(boost::system::error_code const& ec,
                                 message_ptr buff) {
        buff->emplace(std::begin(*buff), boost::asio::zmq::frame(""));
        buff->emplace(std::begin(*buff),
                      boost::asio::zmq::frame(worker_queue_.front()));
        worker_queue_.pop();
