...
private:
    ...
    void handle_write(asio::error_code const& ec) {
        ...
        requester_.async_read_message(
            ...
            std::bind(&req_client::handle_read, this,
                      std::placeholders::_1));
    }
...
public:
    req_client(asio::io_service& ios, asio::zmq::context& ctx, int id)
        : ios_(ios), requester_(ios, ctx, ZMQ_REQ), buffer_() {
        ++count_;
        std::string str_id = std::to_string(id);
        requester_.set_option(
            asio::zmq::socket_option::identity(
                str_id.c_str(), str_id.size()));
        requester_.connect(front_endpoint);

        //  Send request, get reply
        buffer_.push_back(asio::zmq::frame("HELLO"));
        requester_.async_write_message(
            std::begin(buffer_), std::end(buffer_),
            std::bind(&req_client::handle_write, this,
                      std::placeholders::_1));
    }
};
...
private:
    ...
    void handle_write(asio::error_code const& ec) {
        ...
        requester_.async_read_message(
            std::back_inserter(buffer_),
            std::bind(&req_worker::handle_read, this,
                      std::placeholders::_1));
    }

    void handle_read(asio::error_code const& ec) {
        //  Get request, send reply
        asio::zmq::frame instruction = std::move(buffer_.back());
        buffer_.pop_back();
        buffer_.push_back(asio::zmq::frame("OK"));
        std::cout << "Worker: "
        ...
        requester_.async_write_message(
            std::begin(buffer_), std::end(buffer_),
            std::bind(&req_worker::handle_write, this,
                      std::placeholders::_1));
    }
...
public:
    req_worker(asio::io_service& ios, asio::zmq::context& ctx, int id)
        ...
        buffer_.push_back(asio::zmq::frame("READY"));
        requester_.async_write_message(
            std::begin(buffer_), std::end(buffer_),
            std::bind(&req_worker::handle_write, this,
                      std::placeholders::_1));
...
private:
    ...
    void handle_worker_ready(asio::error_code const& ec,
                             message_ptr buff) {
        ...
        if (buff->size() == 5)
            frontend_.async_write_message(
                ...
                std::bind(&lbbroker::null_handler, this,
                          std::placeholders::_1, buff));

        message_ptr frontend_buff {new message_t};
        frontend_.async_read_message(
            std::back_inserter(*frontend_buff),
        ...
        backend_.async_read_message(
            ...
            std::bind(&lbbroker::handle_worker_ready, this,
                      std::placeholders::_1, backend_buff));
    }

    //  Now get next client request, route to LRU worker
    //  Client request is [address][empty][request]
    void handle_client_requested(asio::error_code const& ec,
                                 message_ptr buff) {
        buff->emplace(std::begin(*buff), asio::zmq::frame(""));
        buff->emplace(std::begin(*buff),
                      asio::zmq::frame(worker_queue_.front()));
        worker_queue_.pop();
    ...
    void null_handler(asio::error_code const& ec, message_ptr buff) {}

public:
    lbbroker(asio::io_service& ios, asio::zmq::context& ctx)
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < client_count; ++i)
        clients[i].reset(new req_client(ios, ctx, i));

    for (int i = 0; i < worker_count; ++i)
        workers[i].reset(new req_worker(ios, ctx, i));
