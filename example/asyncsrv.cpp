...
private:
    ...
    static void forward(
        ...
        print(header + std::to_string(buff->back()) + " id " +
              std::to_string(buff->front()));
        sender.async_write_message(
            buff->begin(), buff->end(),
            std::bind(null_handler, std::placeholders::_1, buff));

        message_ptr tmp {new message_t};
        recver.async_read_message(
            std::back_inserter(*tmp),
            std::bind(&async_broker::forward,
                      std::placeholders::_1, tmp,
                      std::ref(recver), std::ref(sender),
                      header));
    }
...
public:
    ...
    void start() {
        ...
        backend_.async_read_message(
            std::back_inserter(*backend_msg),
            std::bind(&async_broker::forward,
                      std::placeholders::_1, backend_msg,
                      std::ref(backend_), std::ref(frontend_),
                      "Sending to frontend "));

        message_ptr frontend_msg {new message_t};
        frontend_.async_read_message(
            std::back_inserter(*frontend_msg),
            std::bind(&async_broker::forward,
                      std::placeholders::_1, frontend_msg,
    ...
    void handle_read(asio::error_code const& ec, message_ptr buff) {
        ...
        for (int i = 0; i < reply_count; ++i) {
            std::this_thread::sleep_for(
                std::chrono::milliseconds(1000 / (1 + std::rand() % 8)));
        ...
        dealer_.async_read_message(
            ...
            std::bind(&async_worker::handle_read, this,
                      std::placeholders::_1, another_msg));
    }
